...
In the past, I've seen Christopher Domas's work. I could tell from the picture it was going to be Christopher Domas related because of this awesome project he made: https://github.com/xoreaxeaxeax/REpsych. But I did not expect it to be movuscated (pick a better name devs!)\
...
## Demov
Demovuscating was annoying, but doable. Long ago, I tried compiling demovuscator (https://github.com/leetonidas/demovfuscator), but failed. So, this time, I searched for a docker container instead, and found it: https://hub.docker.com/r/iyzyi/demovfuscator
Does it have malware? Does it upload my binary to some random website? I don't know, but it works (kindave):
...
2. I ran a diff checker against the letter functions, the only difference is the address it calls, the function name, and most importantly: the value written (THOSE LOOK LIKE CHAR VALUES!):
